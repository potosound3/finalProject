          ...
          <ul class="navbar-nav ml-auto mt-2 mt-lg-0">
            ...
            <li class="nav-item dropdown">
              ...
              <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                <a class="dropdown-item" href="/finalProject/admin/orders">주문내역</a>
                <a class="dropdown-item" href="/finalProject/admin/ordersCancel">주문취소내역</a>
                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="/finalProject/admin/refund">반품내역</a>
              </div>
            </li>
          </ul>
        </div>
...
<div style="display: block; text-align: center;">		
		<c:if test="${paging.startPage != 1 }">
			<a href="/finalProject/admin/orders?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a href="/finalProject/admin/orders?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="/finalProject/admin/orders?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
		</c:if>
	</div>

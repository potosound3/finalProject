        ...
        <c:forEach var="orders" items="${orderList }">
			...
			<div style="display: block; text-align: center;" id="paging">		
					<c:if test="${paging.startPage != 1 }">
						<a href="/finalProject/mypage/orders?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
					</c:if>
					<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
						<c:choose>
							...
							<c:when test="${p != paging.nowPage }">
								<a href="/finalProject/mypage/orders?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
							</c:when>
						</c:choose>
					</c:forEach>
					<c:if test="${paging.endPage != paging.lastPage}">
						<a href="/finalProject/mypage/orders?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
					</c:if>
			</div>
		</div>
	</div>         
